...
module.exports = {
  insertFeedback: function(feedback) {
    ...
    var values = [feedback.rating,feedback.forBusinessUse ? 1 : 0,feedback.email,feedback.comments,feedback.xmin,feedback.ymin,feedback.xmax,
                  feedback.ymax,feedback.centerX,feedback.centerY,feedback.scale,dtString,feedback.otherUses,feedback.education,feedback.recreation,
  ...
  getFeedback: function(id, callback) {
    var sql = `select * from web_search.tbl_os_feedback  where id = $1;`;
    var values = [id];
    const pg = new postgres({ dbName: "tabular" });
